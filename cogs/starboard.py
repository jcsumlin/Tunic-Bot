...
from .utils.Starboard import StarBoard
...
class Star(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.database = StarBoard()

    @commands.group()
    @commands.has_permissions(manage_channels=True)
    async def starboard(self, ctx):
        """(category) Commands for managing the starboard"""
    ...
    async def get_posted_message(self, guild, message, count):
        ...
        if message is not None:
            ...
            if updated_message is not False:
                return updated_message.starboard_message_id
        return None
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if react in str(reaction):
            ...
            for r in msg.reactions:
                if str(r.emoji) in str(reaction):
                    count = r.count

            if await self.check_is_posted(guild, msg): # check if stared message is in starboard
                channel = guild.get_channel(starboard_settings.channel_id)
                msg_id = await self.get_posted_message(guild, msg, count)
                if msg_id is not None:
                    msg_edit = await channel.fetch_message(id=int(msg_id))
                    await msg_edit.edit(content=f"{reaction} **#{count}**")
                    return
            else:
                if count < threshold:
            ...
            em = await self.build_starboard_message(msg, author, channel)
            post_msg = await starboard_channel.send("{} **#{}**".format(str(reaction), count),
                                                   embed=em)
